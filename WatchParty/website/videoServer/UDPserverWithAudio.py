'''
This is the server that a user creates when a user enters a video link and clicks watch this 
This initalizes the sockets needed for video and audio transmission.
The first function is what launches the server and it uses a threadpool executor to run the video and audio strema function 
that transmit the data to the other client connected to the room
'''
...
video_path = 'WatchParty/website/Videos/video.mp4'    
...
#runVideoServer(video_path, audio_path)         #This can be uncommented to just run the client and server code alone 